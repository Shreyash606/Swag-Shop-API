    ...
    WishList.find({}).populate({
        ...
        if (err) {
            response.send(500).send({
                error: "Could nor fetch wishlists"
            });
        } else {
...
app.post('/wishlist', function (request, respnose) {
    ...
    wishList.save(function (err, newWishList) {
        if (err) {
            response.status(500).send({
                error: "Couldnot create wishlist "
            });
        ...
        if (err) {
            ...
            WishList.update({
                ...
                if (err) {
                    ...
                    respone.send("Successfully added");
...
app.listen(3001, function () {
    console.log("Swag shop API is running on port 3001...")
});
